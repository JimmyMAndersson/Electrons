...
#define TOUCH_FORCE_DISTANCE_SQUARED 6400.0
#define TOUCH_CHARGE 300.0
                                    ...
                                    uint index [[thread_position_in_grid]])
{
  ...
  if (info.touch.x > -1 && info.touch.y > -1) {
    float xComp = info.touch.x - electrons[index].position.x;
    float yComp = info.touch.y - electrons[index].position.y;
    float distanceSquared = xComp * xComp + yComp * yComp;
    if (distanceSquared < TOUCH_FORCE_DISTANCE_SQUARED) {
      float angle = precise::atan2(yComp, xComp);
      float2 force = float2(TOUCH_CHARGE * cos(angle) / distanceSquared,
                            TOUCH_CHARGE * sin(angle) / distanceSquared);
