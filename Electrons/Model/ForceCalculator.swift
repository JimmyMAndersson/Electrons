...
final class ForceCalculator {
  ...
  init?(bufferCapacity: Int, device: MTLDevice) {
    ...
    self.info = Info(capacity: 0, maxX: Float(UIScreen.main.bounds.maxX), maxY: Float(UIScreen.main.bounds.maxY), touch: .init(-1, -1))
  ...
  func update(electrons: [Electron], touch: CGPoint?, completion: @escaping ([Electron]) -> ()) {
    ...
    if let touch = touch {
      info.touch = .init(x: Float(touch.x), y: Float(touch.y)) 
    } else {
      info.touch = .init(x: -1, y: -1)
    }
